...
'''
    ...
    copyAssets().then((dataDir) {
      for (String type in types) {
        print('type=$type');
        converters[type] =
            createConverter(kIsWeb ? type : '$dataDir/$type.json');
        print('converter $type = ${converters[type]}');
      }
      if (mounted) {
        setState(() {
          initialized = true;
  ...
  String getType() {
    String originType =
        originTypes[originSelections.indexWhere((element) => element)];
    String targetType =
        targetTypes[targetSelections.indexWhere((element) => element)];
    String variantType =
        variantTypes[variantSelections.indexWhere((element) => element)];
    String idiomType =
        idiomTypes[idiomSelections.indexWhere((element) => element)];
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                type,
                style: TextStyle(
                    color: types.contains(type) ? Colors.green : Colors.red),
              ),
              gutter,
              TextField(
