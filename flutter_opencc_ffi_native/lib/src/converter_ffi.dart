...
class ConverterFfi extends Converter {
  ...
  String convert(String text) {
    ...
    Pointer<Char> resultPtr =
        bindings.opencc_convert(textPtr.cast(), _convertId);
    malloc.free(textPtr);
  ...
  List<String> convertList(List<String> texts) {
    ...
    Pointer<Pointer<Char>> resultPtr =
        bindings.opencc_convert_list(textsPtr.cast(), size, _convertId);
    textPtrList.forEach(malloc.free);
    malloc.free(textsPtr);
    List<String> result = List.generate(
        size, (index) => resultPtr[index].cast<Utf8>().toDartString());
